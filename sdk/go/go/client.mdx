...
1. Use [GetClient](<#GetClient>) to fetch the [MarketDataClient](<#MarketDataClient>) instance and set the API token. 2. Turn on Debug mode to log detailed request and response information to disk as you learn how to use the SDK. 3. Make a test request. 4. Check the rate limit in the client to keep track of your requests. 5. Check the in\-memory logs to see the raw request and response details.
