...
- <a href="#StockQuoteRequest.Get">`Get(...*MarketDataClient) ([]Candle, error)`</a>

  Sends the request, unpacks the response, and returns the data in a user\-friendly format.

- <a href="#StockQuoteRequest.Packed">`Packed(...*MarketDataClient) (*IndicesCandlesResponse, error)`</a>
...
- <a href="#StockQuoteRequest.Raw">`Raw(...*MarketDataClient) (*resty.Response, error)`</a>
...
Raw executes the StockQuoteRequest and returns the raw \*resty.Response. This method optionally accepts a \*MarketDataClient to use for the request, replacing the default client if provided. The \*resty.Response allows access to the raw JSON or \*http.Response for further processing.
