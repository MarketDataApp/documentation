...
Use the `quotes()` method on the `stocks` resource to fetch stock quotes. The method returns a [`MarketDataPromise`](/sdk/js/client#MarketDataPromise) that resolves to decoded records by default, or rejects with a `MarketDataClientError` subclass on failure.
...
- [`MarketDataPromise<StockQuote[] | StockQuoteHuman[] | Blob>`](/sdk/js/client#MarketDataPromise)
