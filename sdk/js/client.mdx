...
- `utilities` (UtilitiesResource): Access to utility endpoints (`status`, `headers`, `user`)
...
| `NotFoundError` | 404 — exported but not thrown by default. 404 resolves to `{s: "no_data"}` instead; see [no-data handling](#NoData). |
