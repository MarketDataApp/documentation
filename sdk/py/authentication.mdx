...
description: Set your Market Data Bearer Token for the Python SDK through an environment variable, a .env file, or by passing it to the client directly.
sidebar_position: 2
---
...
2. Pass it directly when creating the [client](/sdk/py/client)

On startup, the SDK will look for the environment variable. If found, it will use the token for all requests. If the environment variable is not found, you will need to pass the token when creating the [client](/sdk/py/client).
...
After setting the variable, run your code to make a test request. If you have set the variable correctly, you should see the output of the test request. If you see an error, double-check that you have set the variable correctly.
...
The SDK will automatically load this file if you have `python-dotenv` installed.
...
if isinstance(quotes, MarketDataClientErrorResult):
    print(f"Error occurred: {quotes.error}")
else:
    # Print the quotes
    print(quotes)
```
...
If you decide to pass the token directly when creating the client, you can do so by passing it as a parameter. This is not recommended for production code.
...
After successful authentication, you will be able to begin making requests to the Market Data API. We recommend reading the brief overview of how the [client](/sdk/py/client) works, so you can get familiar with interacting with the SDK. The SDK includes methods for each endpoint that allow you to retrieve data. Use any of the endpoints in the SDK to make requests to the API.

You may also want to configure [Settings](/sdk/py/settings) to customize output format, date format, and other universal parameters.
