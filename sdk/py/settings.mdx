---
title: Settings (Python SDK)
sidebar_label: Settings
description: Configure the Python SDK output format, date format and data mode through several methods, and see which priority level wins when they disagree.
sidebar_position: 4
---
...
```python
from marketdata import MarketDataClient, OutputFormat, DateFormat
...
**Example:**
```python
from marketdata import MarketDataClient, Mode
...
The `mode` parameter is available only on paid plans. Free and trial plans cannot change the data mode and receive historical data (the last fully-closed session). See [Data Freshness](/account/data-freshness).
:::
...
- `MARKETDATA_MODE`: Data mode (`live`, `cached`, `delayed`)
