...
try (MarketDataClient client = new MarketDataClient()) {
    OptionQuote q =
        client.options().quote(OptionsQuoteRequest.of("AAPL271217C00250000")).values().get(0);
    System.out.println(
        q.optionSymbol() + ": bid=" + q.bid() + " ask=" + q.ask() + " delta=" + q.delta());
...
MarketDataClient().use { client ->
    val q =
        client.options().quote(OptionsQuoteRequest.of("AAPL271217C00250000")).values()[0]
    println("${q.optionSymbol()}: bid=${q.bid()} ask=${q.ask()} delta=${q.delta()}")
}
```
