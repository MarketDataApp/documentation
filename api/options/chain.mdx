...
- **strikeLimit** `number`
 ...
 :::tip
 If `strikeLimit` is combined with the `range` parameter, range will be applied first. In the above example, if the range were set to `itm` (in the money), all out of the money strikes would be first excluded by the range parameter and then strikeLimit will return a maximum of 10 in the money strikes that are closest to the money. If the `side` parameter has not been used, then `strikeLimit` will return the requested number of strikes for each side of the chain, duplicating the number of strikes that are received.
 :::
