...
## Credits
Each time you call the API, the system increases your credits counter. Normally each successful response consumes 1 credit. However, **if you request multiple symbols in a single API call using bulkquotes, bulkcandles, or option chain, credits are consumed per symbol included in the response**.
...
- `X-Api-Ratelimit-Limit`: The maximum number of API credits permitted (per day for Free/Starter/Trader plans or per minute for Prime users).
