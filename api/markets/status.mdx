...
public class MarketStatusApp {
  public static void main(String[] args) {
    try (MarketDataClient client = new MarketDataClient()) {
      List<MarketStatus> days = client.markets()
          .status(MarketStatusRequest.builder()
              .from(LocalDate.of(2020, 1, 1))
              .to(LocalDate.of(2020, 12, 31))
              .build())
          .values();
      for (MarketStatus d : days) {
        System.out.printf("date=%s status=%s%n", d.date(), d.status());
      }

      List<MarketStatus> today = client.markets()
...
fun main() {
    MarketDataClient().use { client ->
        val days = client.markets()
            ...
            .values()
        for (d in days) {
            println("date=${d.date()} status=${d.status()}")
        }

        val today = client.markets()
            .status(MarketStatusRequest.of())
            .values()
        println(today[0])
    }
