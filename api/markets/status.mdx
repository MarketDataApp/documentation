...
**GET** [https://api.marketdata.app/v1/markets/status/?from=2024-01-01&to=2024-01-31](https://api.marketdata.app/v1/markets/status/?from=2024-01-01&to=2024-01-31)
...
try {
  const days = await client.markets.status({ from: "2024-01-01", to: "2024-01-31" });
  for (const d of days) {
...
try {
  const days: MarketStatusResponse = await client.markets.status({ from: "2024-01-01", to: "2024-01-31" });
  for (const d of days) {
    console.log(`date=${d.date} status=${d.status}`);
  }
...
status1 = client.markets.status(from_date="2024-01-01", to_date="2024-01-31")
...
func ExampleMarketStatus() {
	msr, err := api.MarketStatus().From("2024-01-01").To("2024-01-31").Get()
	if err != nil {
		fmt.Print(err)
		return
	}
...
```csharp title="MarketStatusApp.cs"
using MarketDataApp;

using var client = await MarketDataClient.CreateAsync();

var january = await client.Markets.GetStatusAsync(
    from: new DateOnly(2024, 1, 1),
    to: new DateOnly(2024, 1, 31));
foreach (var day in january.Values)
{
    Console.WriteLine(day);
}

var yesterday = await client.Markets.GetStatusAsync(
    date: DateOnly.FromDateTime(DateTime.Today.AddDays(-1)));
foreach (var day in yesterday.Values)
{
    Console.WriteLine(day);
}
```
