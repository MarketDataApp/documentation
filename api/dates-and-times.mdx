...
description: How the Market Data API formats response timestamps, which timezone it reports them in, and which date formats a request accepts.
sidebar_position: 5
---
...
Option-related keyphrases cannot be used to return expiration dates far in the future for options which have not yet been traded or for options in the past which have already expired. For example, if today is January 15, 2023, you couldn't use `November 2023's 1st weekly expiration` since weekly options for November would not exist yet. The API will return a `No data` response if you try to request an expiration that does not exist, whether in the future or the past.
