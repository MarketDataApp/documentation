...
candles = client.funds.candles("VFINX", resolution="D", from_date="2020-01-01", to_date="2020-01-10")
print(candles)
```
</TabItem>
...
fun main() {
    MarketDataClient().use { client ->
        ...
        val candles = client.funds()
            ...
            .values()
        for (c in candles) {
            println("t=${c.time()} o=${c.open()} h=${c.high()} l=${c.low()} c=${c.close()}")
        }
    }
}
```
