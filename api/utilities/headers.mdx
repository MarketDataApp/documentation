...
public class HeadersCheck {
  public static void main(String[] args) {
    try (MarketDataClient client = new MarketDataClient()) {
      Map<String, String> h = client.utilities().headers().values();
      System.out.println(h.get("user-agent"));
      System.out.println(h.get("accept-encoding"));
    }
  }
}
```
...
fun main() {
    MarketDataClient().use { client ->
        val h = client.utilities().headers().values()
        println(h["user-agent"])
        println(h["accept-encoding"])
    }
}
```
