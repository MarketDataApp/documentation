...
description: Retrieve the headers your application sends to the Market Data API as JSON, to debug an Authorization header that is not working.
sidebar_position: 2
---
...
```csharp title="HeadersCheck.cs"
using MarketDataApp;

using var client = await MarketDataClient.CreateAsync();

foreach (var (name, value) in (await client.Utilities.GetHeadersAsync()).Values)
{
