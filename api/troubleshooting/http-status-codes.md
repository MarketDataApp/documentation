...
- `400 BAD REQUEST` - Input could not be parsed or validated (for example URL construction issues, invalid parameter values/types, invalid date usage, or invalid option symbols). See [400: Bad Request](/api/troubleshooting/bad-request) for the complete troubleshooting guide and [URL Anatomy (Path vs Query Parameters)](/api/troubleshooting/url-parameters) for URL construction basics.
