...
- `400 BAD REQUEST` - Invalid date usage can also return 400 (for example, using `date` in a current-data request when `date` is historical-only). See [URL Parameters](/api/troubleshooting/url-parameters).
