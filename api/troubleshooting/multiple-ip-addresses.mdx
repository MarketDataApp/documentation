...
description: A 403 means the account sent requests from more than one IP address. The block clears itself after five minutes with no requests.
sidebar_position: 7
---
...
4. **Serverless Functions Are Not Supported (Lambda, Google Cloud Functions, Google Apps Script, etc.)**: Your subscription is licensed to run on a **single device**. Serverless platforms execute your code across a pool of ephemeral instances with rotating outbound IP addresses, which inherently bypasses the single-device licensing terms. For this reason, serverless functions are not compatible with the API and are not a supported deployment model. Run your workload on a single, persistent device or instance with a stable IP address instead.

5. **Multiple Environments (Development, Staging, Production)**: If you're running more than one environment that calls the API at the same time using the same token, you will hit this block. Sharing the token across environments is also a token-sharing violation under the [Single IP Address Policy](/docs/account/data-policies/single-ip/).

   Compliant options:
   - **Serialize**: run only one environment at a time. For most testing and A/B workflows, running scenarios sequentially is sufficient.
   - **Use the no-auth demo symbols in development**: stock and option endpoints accept **AAPL** (and any AAPL option contract, e.g. `AAPL271217C00250000`) with no token at all. Development and integration tests that exercise the API surface — request shape, response parsing, error handling — can use these symbols without ever touching your account. See [Authentication › Demo The API With No Authentication](/docs/api/authentication/#demo-the-api-with-no-authentication). Note that no-auth requests return historical data only.
   - **Commercial license**: if your use case genuinely requires simultaneous parallel data across multiple environments (not testing), contact sales — that case requires a commercial license and our sales team will assist.

## Important Note

The system is designed to detect patterns of simultaneous access, not just IP changes. While you are permitted to change devices, attempting to maintain multiple active connections from different computers — even when egressed through a single shared IP — will result in account blocks and may result in suspension.
