...
```kotlin title="StockQuote.kt"
import com.marketdata.sdk.MarketDataClient
import com.marketdata.sdk.stocks.StockQuoteRequest

MarketDataClient().use { client ->
    client.stocks().quote(StockQuoteRequest.of("AAPL")).values().forEach(::println)
}
```
...
```csharp title="StockQuote.cs"
using MarketDataApp;

using var client = await MarketDataClient.CreateAsync();

foreach (var quote in (await client.Stocks.GetQuoteAsync("AAPL")).Values)
{
...
func ExampleBulkStockQuotesRequest() {
	quotes, err := api.BulkStockQuotes().Symbols([]string{"AAPL", "META", "MSFT"}).Get()
	if err != nil {
		fmt.Print(err)
		return
	}
...
```kotlin title="StockQuotes.kt"
import com.marketdata.sdk.MarketDataClient
import com.marketdata.sdk.stocks.StockQuotesRequest

MarketDataClient().use { client ->
    client.stocks().quotes(StockQuotesRequest.of("AAPL", "META", "MSFT")).values().forEach(::println)
}
```
...
```csharp title="StockQuotes.cs"
using MarketDataApp;

using var client = await MarketDataClient.CreateAsync();

foreach (var quote in (await client.Stocks.GetQuotesAsync(["AAPL", "META", "MSFT"])).Values)
{
    Console.WriteLine(quote);
}
```
