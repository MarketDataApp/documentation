...
try (MarketDataClient client = new MarketDataClient()) {
    var quotes = client.stocks().quotes(
        StockQuotesRequest.builder("AAPL", "META", "MSFT").build());
    for (StockQuote q : quotes.values()) {
        System.out.printf("%s: bid=%s, ask=%s, last=%s%n",
...
MarketDataClient().use { client ->
    ...
    for (q in quotes.values()) {
        println("${q.symbol()}: bid=${q.bid()}, ask=${q.ask()}, last=${q.last()}")
    }
}
```
