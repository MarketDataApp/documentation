...
candles = client.stocks.candles("AAPL", resolution="D", from_date="2020-01-01", to_date="2020-12-31")
print(candles)
```
</TabItem>
...
Validation runs on the *resolved* flag values (after the daily/intraday defaults are applied), so a request that omits a flag is validated against the default it resolved to.
