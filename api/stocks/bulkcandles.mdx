...
## Endpoint
    ...
    https://api.marketdata.app/v1/stocks/bulkcandles/{resolution}/{date}/
...
```mdx-code-block
<Tabs>
<TabItem value="required" label="Optional" default>

```

- **date** `datekey`

  The date of the candle to be returned in ISO 8601 (YYYY-MM-DD) format. If no date is provided, the current day is assumed. The leftmost candle on a chart (inclusive). If you use countback, to is not required. Accepted timestamp inputs: ISO 8601, unix, spreadsheet.
