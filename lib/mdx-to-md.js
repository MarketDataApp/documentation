...
function resolveRelative(sourcePath, sdkBase, targetSub) {
  ...
  const cleaned = targetSub.replace(/\/$/, '');
  const target = cleaned === '' ? 'index' : cleaned;
  return up + target + '.md';
}
...
function rewriteHref(href, opts) {
  ...
  const { sourcePath, sdk, baseUrl } = opts;
  ...
  if (sdk && sourcePath) {
    ...
    if (pathPart === sdkRoot || pathPart === sdkRoot + '/') {
      return resolveRelative(sourcePath, `sdk/${sdk}`, '') + anchor;
    }
    if (pathPart.startsWith(sdkPrefix)) {
      const sub = pathPart.slice(sdkPrefix.length);
      return resolveRelative(sourcePath, `sdk/${sdk}`, sub) + anchor;
    }
  }
