...
// Render a <DocCardList> as a plain-markdown bullet list of links. Docusaurus
// turns the JSX into navigable cards; GitHub-flavored .md has no equivalent, so
// without this the endpoint list on every section index page would just vanish.
// Falls back to '' (drop the component) when the caller supplies no items —
// e.g. the worker, which serves a single file and can't see its siblings.
function renderCardList(opts) {
  const items = opts.cardItems;
  if (!Array.isArray(items) || items.length === 0) return '';
  return items
    .map(({ label, href, description }) =>
      description ? `- [${label}](${href}) — ${description}` : `- [${label}](${href})`
    )
    .join('\n');
}
...
function transformProse(text, opts) {
  ...
  const cards = renderCardList(opts);
  text = text.replace(DOCCARDLIST_PAIRED_RE, cards ? `${cards}\n` : '');
  text = text.replace(DOCCARDLIST_SELF_RE, cards ? `${cards}\n` : '');
...
function cleanMdx(text, opts = {}) {
  const o = {
    ...
    // Ordered [{ label, href, description? }] used to render <DocCardList> as a
    // markdown link list. CLI computes this per index file from its siblings;
    // unset callers fall back to dropping the component.
    cardItems: opts.cardItems ?? null,
  };
