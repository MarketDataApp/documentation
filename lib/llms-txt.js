 ...
 *
 * ---------------------------------------------------------------------------
 * THE ATTRIBUTE VALUE MAY BE UNQUOTED, AND REQUIRING QUOTES FAILED SILENTLY
 * ---------------------------------------------------------------------------
 *
 * This read `name="description"` and `content="..."` with the quotes written
 * in. Under `future.v4` the Faster pipeline minifies harder and emits
 * `name=description` for any value with nothing in it needing a quote. The
 * match then failed on EVERY page and llms.txt went from 55 KB to 23 KB --
 * every one of its 259 entries losing its description, with no error.
 *
 * Nothing caught it. It is still a valid index: one H1 first, no H6, every
 * line reaching /docs/, so both this repo's build-contract check and the
 * orchestrator's own splice preconditions passed. Only diffing the file
 * against the previous build showed it.
 *
 * So all three HTML5 forms are accepted -- double-quoted, single-quoted and
 * bare -- which is what lib/not-found-head.js's `attributesOf` has always
 * done, and why the noindex and 404 head rewriters came through the same
 * change untouched.
 */
const META_DESCRIPTION =
  /<meta[^>]*\bname\s*=\s*(?:"description"|'description'|description)[^>]*\bcontent\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s"'`=<>]+))/i;
...
/**
 * Whether a skipped route is a navigation artifact rather than content.
 *
 * categoryOf returns null for two different reasons: the route is a tag page,
 * the search UI or the 404 -- all deliberate -- OR its top-level directory is
 * one this file has never heard of, which is a new docs section silently
 * missing from the index.
 *
 * The build log used to call both "navigation artifact(s) skipped", counting
 * routes-minus-entries. That number was always correct and the label was not:
 * a whole new section would have been reported as housekeeping. Splitting the
 * two lets the caller fail on the second.
 */
/**
 * Whether a built page tells crawlers not to index it.
 *
 * Read from the RENDERED page rather than from a list of routes, and that is
 * the whole design. Docusaurus can set `noindex` from `siteConfig.noIndex`,
 * from a page's front matter, or from plugin config; a list keyed to any one
 * of them misses the others, and a second list is a second thing to keep in
 * step. The rendered `<meta>` is where all three converge.
 *
 * Parsed with domino rather than matched with a regex. A minifier is free to
 * write `<meta content="noindex" name=robots>` -- quoted on one attribute,
 * bare on the other -- and a matcher requiring quotes reads every page as
 * having no robots tag, which is indistinguishable from a page that has none.
 * That cost MarketDataApp/website a release: 56 checks passed against an
 * unminified dev server and every browser check failed in CI, which serves the
 * build. A DOM cannot be fooled by quoting or attribute order.
 */
function isNoindexHtml(html) {
  if (!html) return false;
  const doc = domino.createWindow(html).document;
  const robots = doc.querySelector('meta[name="robots"]');
  return /noindex/i.test(robots?.getAttribute('content') ?? '');
}

function isNavigationArtifact(stem) {
  if (categoryOf(stem)) return false;
  const parts = stem.split('/');
  return (
    parts.includes('tags') ||
    // `internal/` is our own reference material and never belongs in an index
    // written for an outside consumer.
    //
    // It is excluded HERE, by stem, rather than left to the noindex withholding
    // in plugins/llms-txt.js -- even though every page under it also carries a
    // noindex directive. That directive is stamped by `plugins/internal-head.js`
    // in postBuild, and Docusaurus runs postBuild hooks with `Promise.all`, so
    // whether this file's caller reads a stamped or an unstamped page is a
    // race. Classifying by stem is decided before any of that and cannot lose
    // it. The noindex tag is for crawlers; this line is for llms.txt.
    parts[0] === 'internal' ||
    stem === 'search' ||
    /^404(\.html)?$/.test(stem)
  );
...
module.exports = { categoryOf, isNavigationArtifact, isNoindexHtml, renderIndex, renderFull, disambiguate, twinUrl, assertNoSpliceMarker, SPLICE_MARKER, titleForStem, ROOT_TITLE, titleFromMarkdown, descriptionFromHtml, SDK_LANGUAGES, SECTION_ORDER };
