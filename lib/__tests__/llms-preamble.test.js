...
// The URLs are stripped before asserting on the prose, and that is the whole
// point of this helper. `assert.match(PREAMBLE, /index\.md/)` looks like it
// tests that the convention is stated; it does not. The authentication URL
// ends in `index.md`, so the assertion passes on a preamble whose prose has
// been deleted entirely. raven hit the identical defect in lint:twins the same
// afternoon: a sound instrument answering an adjacent question.
const prose = () =>
  PREAMBLE.split('\n')
...
test('the preamble still says how to reach the HTML page', () => {
  assert.match(prose(), /drop the\s+trailing `index\.md`/);
});

test('that assertion is not vacuous: it fails when the sentence is removed', () => {
  const gutted = PREAMBLE.replace(/Every link below[\s\S]*?trailing `index\.md`\.\n\n/, '')
    .split('\n')
    .filter((line) => !line.trim().startsWith('http'))
    .join('\n');
  assert.doesNotMatch(gutted, /drop the\s+trailing `index\.md`/);
});
