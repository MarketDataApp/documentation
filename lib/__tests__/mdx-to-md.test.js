...
/**
 * The SEO work adds a hand-written `description:` to ~150 pages. These guard
 * the one thing that would make that unsafe: the description reaching the
 * Markdown twin as body text, so every agent reading a twin sees the summary
 * repeated above the article it summarises.
 *
 * Proven end to end before the bulk edit began, on
 * account/troubleshooting/account-deletion.md: adding a description changed
 * the meta tag and the llms.txt entry, and left the twin byte-identical.
 */
test('strips a description from frontmatter rather than emitting it', () => {
  const input = '---\ntitle: T\ndescription: A summary of the page.\n---\n\nBody.\n';
  assert.equal(cleanMdx(input), '# T\n\nBody.\n');
});

test('strips descriptions containing the characters prose actually uses', () => {
  // A colon forces YAML quoting, an em dash and a comparison sign are common in
  // this corpus, and an apostrophe is the classic quote-nesting mistake.
  for (const desc of [
    '"Option chain: strikes, greeks and IV."',
    "The API's option chain endpoint",
    'Strikes, greeks — and IV, ≥15 minutes delayed',
    `"A description with 'nested' quotes"`,
  ]) {
    const input = `---\ntitle: T\ndescription: ${desc}\n---\n\nBody.\n`;
    assert.equal(cleanMdx(input), '# T\n\nBody.\n', `leaked for: ${desc}`);
  }
...
test('strips a folded multi-line description', () => {
  const input = '---\ntitle: T\ndescription: >-\n  A folded description\n  over two lines.\n---\n\nBody.\n';
  assert.equal(cleanMdx(input), '# T\n\nBody.\n');
});

test('an indented --- inside a block scalar does not end the frontmatter early', () => {
  // The only shape that could leak: a closing delimiter found too soon would
  // push the rest of the frontmatter into the body. YAML requires the inner
  // line to be indented, which is what keeps `^---` from matching it.
  const input = '---\ntitle: T\ndescription: |\n  Line one\n  ---\n  Line two\n---\n\nBody.\n';
  const out = cleanMdx(input);
  assert.equal(out, '# T\n\nBody.\n');
  assert.ok(!out.includes('description:'), 'frontmatter leaked into the body');
});
...
test('renders <DocCardList /> as a markdown link list when cardItems supplied', () => {
  const input = '# Stocks\n\n<DocCardList items={useCurrentSidebarCategory().items} />\n';
  const result = cleanMdx(input, {
    cardItems: [
      { label: 'Prices', href: './prices.md' },
      { label: 'Candles', href: './candles.md' },
    ],
  });
  assert.ok(!result.includes('DocCardList'));
  assert.match(result, /- \[Prices\]\(\.\/prices\.md\)/);
  assert.match(result, /- \[Candles\]\(\.\/candles\.md\)/);
  // Order is preserved as given (caller pre-sorts by sidebar_position).
  assert.ok(result.indexOf('Prices') < result.indexOf('Candles'));
});

test('renders card descriptions when present', () => {
  const input = '<DocCardList items={useCurrentSidebarCategory().items} />\n';
  const result = cleanMdx(input, {
    cardItems: [{ label: 'Candles', href: './candles.md', description: 'OHLC data' }],
  });
  assert.match(result, /- \[Candles\]\(\.\/candles\.md\) — OHLC data/);
});

test('empty cardItems falls back to dropping the component', () => {
  const input = '# Stocks\n\n<DocCardList items={useCurrentSidebarCategory().items} />\n';
  assert.ok(!cleanMdx(input, { cardItems: [] }).includes('DocCardList'));
});
