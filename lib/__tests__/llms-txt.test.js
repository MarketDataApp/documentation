...
test('an api route lands in the API section with no subsection', () => {
  assert.deepEqual(categoryOf('api/stocks/candles'), { section: 'API reference', subsection: null });
});
...
test('sheets and account routes land in their own sections', () => {
  assert.deepEqual(categoryOf('sheets/stocks/stockdata'), {
    section: 'Google Sheets reference',
    subsection: null,
  });
...
test('a section with no entries is left out entirely', () => {
  ...
  assert.doesNotMatch(out, /## Google Sheets reference/);
});
...
// future.v4's Faster pipeline emits unquoted attributes wherever the value
// needs no quote. Requiring quotes cost llms.txt every one of its 259
// descriptions -- 55 KB to 23 KB -- and no check went red, because an index
// with no descriptions is still a structurally valid index.
test('a description is read whether the attributes are quoted or not', () => {
  const bare = '<meta name=description content=Candles>';
  assert.equal(descriptionFromHtml(bare), 'Candles');

  const single = "<meta name='description' content='Get candles.'>";
  assert.equal(descriptionFromHtml(single), 'Get candles.');

  // The realistic minified shape: name bare, content quoted because it has
  // spaces. This is what 3.10 + future.v4 actually writes.
  const mixed = '<meta name=description content="Get historical candles.">';
  assert.equal(descriptionFromHtml(mixed), 'Get historical candles.');

  // A bare value stops at the tag, and must not swallow the next attribute.
  const twoAttrs = '<meta name=description content=Candles data-rh=true>';
  assert.equal(descriptionFromHtml(twoAttrs), 'Candles');
});
...
test('a real page whose name merely contains "tags" is kept', () => {
  assert.deepEqual(categoryOf('api/tagsoup'), { section: 'API reference', subsection: null });
  assert.deepEqual(categoryOf('api/stocks/tags-explained'), { section: 'API reference', subsection: null });
});
...
// focus's failure mode 5, from our side. llms-full.txt is the concatenated
// body of every page, so any page that ever quotes the splice marker -- a page
// documenting this very mechanism would -- puts a second marker into the file
// the orchestrator splices. Ours must never carry one.

const { assertNoSpliceMarker, SPLICE_MARKER } = require('../llms-txt');

test('the marker constant is the one the orchestrator looks for', () => {
  assert.equal(SPLICE_MARKER, '<!-- docs:llms -->');
});

test('content carrying the splice marker is refused, naming the file', () => {
  assert.throws(
    () => assertNoSpliceMarker('# A page\n\n<!-- docs:llms -->\n', 'llms-full.txt'),
    /llms-full\.txt/
  );
});

test('a marker written with different spacing is caught too', () => {
  assert.throws(() => assertNoSpliceMarker('<!--docs:llms-->', 'llms.txt'), /marker/i);
  assert.throws(() => assertNoSpliceMarker('<!--   docs:llms   -->', 'llms.txt'), /marker/i);
});

test('ordinary content passes', () => {
  assert.doesNotThrow(() => assertNoSpliceMarker('# CORS\n\nBody.\n', 'llms.txt'));
});

// --- disambiguation ---
//
// 252 entries carry only 130 distinct titles, and 14 pairs collide INSIDE a
// single section, where the heading cannot tell them apart: "Candles" under
// "### Go" is both funds/candles and stocks/candles. raven raised this shape
// on their own file; ours turned out to have it worse. The parent path segment
// is what separates them.

const { disambiguate } = require('../llms-txt');

test('a title colliding inside its section is qualified by its parent segment', () => {
  const out = disambiguate([
    { stem: 'sdk/go/funds/candles', title: 'Candles' },
    { stem: 'sdk/go/stocks/candles', title: 'Candles' },
  ]);
  assert.deepEqual(out.map((e) => e.title), ['Candles (Funds)', 'Candles (Stocks)']);
});

test('a title that is unique in its section is left alone', () => {
  const out = disambiguate([
    { stem: 'sdk/go/stocks/candles', title: 'Candles' },
    { stem: 'sdk/go/options/chain', title: 'Option Chain' },
  ]);
  assert.deepEqual(out.map((e) => e.title), ['Candles', 'Option Chain']);
});

test('the same title in two different sections is not a collision', () => {
  const out = disambiguate([
    { stem: 'sdk/go/stocks/candles', title: 'Candles' },
    { stem: 'sdk/py/stocks/candles', title: 'Candles' },
  ]);
  assert.deepEqual(out.map((e) => e.title), ['Candles', 'Candles']);
});

test('a hyphenated parent segment is humanised', () => {
  const out = disambiguate([
    { stem: 'api/universal-parameters/headers', title: 'Headers' },
    { stem: 'api/utilities/headers', title: 'Headers' },
  ]);
  assert.deepEqual(out.map((e) => e.title), [
    'Headers (Universal Parameters)',
    'Headers (Utilities)',
  ]);
});

test('entries the index skips are passed through untouched', () => {
  const out = disambiguate([{ stem: 'api/tags/x', title: 'Tags' }]);
  assert.deepEqual(out.map((e) => e.title), ['Tags']);
});

// focus measured 199 shell comments inside fenced code blocks in the corpus
// that a naive /^#/ pass would read as headings. Every twin happens to open
// with its title today, so the first match is always line 1 -- but that is a
// property of the current content, not a guarantee. Anchor to the first line
// so a page that ever opens with prose cannot take its title from a code
// sample.

test('a title is only taken from the first line, never from later in the page', () => {
  const md = 'Intro paragraph, no heading.\n\n```bash\n# install the SDK\nnpm i\n```\n';
  assert.equal(titleFromMarkdown(md), '');
});

test('a heading below the first line is not the title either', () => {
  assert.equal(titleFromMarkdown('Intro prose.\n\n# Later Heading\n'), '');
});

test('the first line still wins when it is a heading', () => {
  assert.equal(titleFromMarkdown('# CORS\n\n```bash\n# not a title\n```\n'), 'CORS');
});

test('renderFull does not treat a fenced comment as the page heading', () => {
  const out = renderFull({
    origin: ORIGIN,
    entries: [{ stem: 'api/x', markdown: 'Prose first.\n\n```bash\n# nope\n```\n' }],
  });
  assert.match(out, /^Source: \S+\/api\/x\/index\.md\n\nProse first\./m);
  assert.doesNotMatch(out, /^# nope\n\nSource:/m);
});

// A regression I shipped: rewriting renderFull to reuse titleFromMarkdown, I
// took "everything after the first newline" as the body. A single-line page has
// no newline, so indexOf returns -1, slice(0) returns the whole string, and the
// heading was emitted twice. Three real pages are single-line -- the generated
// Sheets category indexes, whose entire body is their heading -- which put 50
// bytes of duplicate headings into the published corpus.

test('a single-line page does not repeat its heading as its body', () => {
  const out = renderFull({
    origin: ORIGIN,
    entries: [{ stem: 'sheets/stocks', markdown: '# Stock Formulas' }],
  });
  assert.equal(
    out,
    '# Stock Formulas\n\nSource: https://www.marketdata.app/docs/sheets/stocks/index.md\n'
  );
});

test('a heading followed only by a trailing newline behaves the same', () => {
  const out = renderFull({
    origin: ORIGIN,
    entries: [{ stem: 'sheets/options', markdown: '# Option Formulas\n' }],
  });
  assert.doesNotMatch(out, /Option Formulas[\s\S]*Option Formulas/);
});

test('a page with a body still keeps it', () => {
  const out = renderFull({
    origin: ORIGIN,
    entries: [{ stem: 'api/cors', markdown: '# CORS\n\nBody text.\n' }],
  });
...
// The build log said "N navigation artifact(s) skipped", where N was simply
// routes-minus-entries. categoryOf returns null for the tag pages, the search
// UI and the 404 -- and equally for any top-level directory it does not know.
// So a new docs section would vanish from the index and be reported as a
// navigation artifact: a correct number under a label that means more than the
// code knows, which is the one shape looking harder at the output cannot catch.

const { isNavigationArtifact } = require('../llms-txt');

test('the real navigation artifacts are recognised as such', () => {
  assert.equal(isNavigationArtifact('search'), true);
  assert.equal(isNavigationArtifact('404.html'), true);
  assert.equal(isNavigationArtifact('api/tags'), true);
  assert.equal(isNavigationArtifact('sheets/tags/sheets-premium'), true);
});

test('an unrecognised section is NOT an artifact, however it is skipped', () => {
  assert.equal(isNavigationArtifact('guides/getting-started'), false);
  assert.equal(isNavigationArtifact('education/options'), false);
});

test('an indexed route is not an artifact either', () => {
  assert.equal(isNavigationArtifact('api/cors'), false);
  assert.equal(isNavigationArtifact(''), false);
});
