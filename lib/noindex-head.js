 ...
 *
 * `noindex` marks a page that must carry the directive. Every page then loses
 * its canonical if -- after that -- it says `noindex`.
 *
 * The caller decides which pages qualify; see `NOINDEX_ROOTS` in
 * `plugins/noindex-head.js`. It is a SET of roots rather than one directory
 * because two unrelated sections need it for unrelated reasons: `/internal/`
 * is our own reference material, and `/search/` is a query-driven UI with no
 * content of its own.
 */
function applyHeadRules(html, { noindex = false, internal = false } = {}) {
  // `internal` is the old spelling of this argument, kept working so a caller
  // that has not been updated does not silently stop stamping anything.
  noindex = noindex || internal;
